...
static jlong native_init(JNIEnv *env, jstring j_det_model_path,
                         jstring j_rec_model_path, jstring j_cls_model_path,
                         jint j_use_opencl, jint j_thread_num,
                         jstring j_cpu_mode);
static jfloatArray native_forward(JNIEnv *env, jlong java_pointer,
                                  jobject original_image, jint j_max_size_len,
                                  jint j_run_det, jint j_run_cls,
                                  jint j_run_rec);
static void native_release(jlong java_pointer);

extern "C" JNIEXPORT jlong JNICALL
Java_com_silentauto_paddlerocr_PaddleOcrNative_init(
  ...
  return native_init(env, j_det_model_path, j_rec_model_path, j_cls_model_path,
                     j_use_opencl, j_thread_num, j_cpu_mode);
}
...
str_to_cpu_mode(const std::string &cpu_mode) {
  ...
  std::string upper_key(cpu_mode.size(), '\0');
...
Java_com_silentauto_paddlerocr_PaddleOcrNative_forward(
  ...
  return native_forward(env, java_pointer, original_image, j_max_size_len,
                        j_run_det, j_run_cls, j_run_rec);
}
...
static jfloatArray native_forward(JNIEnv *env, jlong java_pointer,
  ...
  try {
    results = ppredictor->infer_ocr(origin, max_size_len, run_det, run_cls,
                                    run_rec);
  } catch (const cv::Exception &e) {
    LOGE("infer_ocr OpenCV failed: %s", e.what());
    return cpp_array_to_jfloatarray(env, nullptr, 0);
  } catch (const std::exception &e) {
    LOGE("infer_ocr failed: %s", e.what());
    return cpp_array_to_jfloatarray(env, nullptr, 0);
  } catch (...) {
    LOGE("infer_ocr failed with unknown exception");
    return cpp_array_to_jfloatarray(env, nullptr, 0);
  }
...
Java_com_silentauto_paddlerocr_PaddleOcrNative_release(
    JNIEnv *env, jobject thiz, jlong java_pointer) {
  native_release(java_pointer);
}
