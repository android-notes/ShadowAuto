...
void OCR_PPredictor::infer_rec(const cv::Mat &origin_img, int run_cls,
  ...
  if (crop_img.empty() || crop_img.rows <= 0 || crop_img.cols <= 0) {
    LOGE("ocr cpp rec skip empty crop");
    return;
  }

  if (run_cls) {
    ClsPredictResult cls_res = infer_cls(crop_img);
    if (!cls_res.img.empty()) {
  ...
  if (input_image.empty() || input_image.rows <= 0 || input_image.cols <= 0) {
    LOGE("ocr cpp rec skip empty resize");
    return;
  }
  input_image.convertTo(input_image, CV_32FC3, 1 / 255.0f);
  const float *dimg = reinterpret_cast<const float *>(input_image.data);
...
ClsPredictResult OCR_PPredictor::infer_cls(const cv::Mat &img, float thresh) {
  ...
  if (img.empty() || img.rows <= 0 || img.cols <= 0) {
    LOGE("ocr cpp cls skip empty image");
    return res;
  }
  std::vector<float> mean = {0.5f, 0.5f, 0.5f};
  std::vector<float> scale = {1 / 0.5f, 1 / 0.5f, 1 / 0.5f};
  ...
  if (input_image.empty() || input_image.rows <= 0 || input_image.cols <= 0) {
    LOGE("ocr cpp cls skip empty resize");
    return res;
  }
  input_image.convertTo(input_image, CV_32FC3, 1 / 255.0f);
  const float *dimg = reinterpret_cast<const float *>(input_image.data);
